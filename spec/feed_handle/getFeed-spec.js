...
describe('getFeed', function () {
    ...
    it('should pass most calls straight through to mongoFeedStore', function () {
        expect(getFeed.get).toBe(mongoFeedStore.getMongoFeedData);
        expect(getFeed.getMeta).toBe(mongoFeedStore.getMongoFeedMeta);
        expect(getFeed.getItems).toBe(mongoFeedStore.getMongoFeedItems);
        expect(getFeed.getItemsByID).toBe(mongoFeedStore.getMongoFeedItemsByID);
        expect(getFeed.setRead).toBe(mongoFeedStore.setRead);
    });
    ...
    describe('.add', function () {
        it('should go to the source if not in the database (and store the result)', function (done) {
            ...
            spyOn(mongoFeedStore, 'getMongoFeedData').and.callFake(function () {
                return Promise.resolve(db_contents);
            });
            ...
            .then(function (result) {
                ...
                expect(mongoFeedStore.getMongoFeedData.calls.count()).toEqual(1);
                expect(mongoFeedStore.getMongoFeedData).toHaveBeenCalledWith('http://blog.rss');
