...
describe('get_all', function () {
    ...
    describe('get_all with updated_since query paramter', function () {
        afterAll(util.resetSpies);
        afterAll(util.clearListner);
    ...
    describe('get_all with label query paramter', function () {
        afterAll(util.resetSpies);
        afterAll(util.clearListner);
        ...
        beforeAll(function (done) {
            this.updated_since =  new Date('2000-01-01 12:30');
            this.spyRes.events.once('responseComplete', done);
            get_all({user: {_id: 'IMPOSTER'}, query: {label: 'foo'}}, this.spyRes);
        });

        it('should only return data for that label', function () {
            var data = this.spyRes.json.calls.argsFor(0)[0];
            expect(data.meta.length).toEqual(1);
            expect(data.meta[0].title).toEqual(test_data.meta[1].title);
            expect(data.items.length).toEqual(1);
            expect(data.items[0].link).toEqual(test_data.item[1].link);
        });
    });

    describe('get_all with older_than query paramter', function () {
