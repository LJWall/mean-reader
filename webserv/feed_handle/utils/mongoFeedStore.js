...
module.exports.updateMongoFeedData = function (db, feed_data, user_id) {
    var ret = [], m;
    return Promise.resolve(db)  /* Agnostic on getting passed a db or */
    .then(function (_m) {       /* a promise of one.                  */
        m = _m;
        var match = {feedurl: feed_data.meta.feedurl, user_id: user_id};
        return m.collection('feeds').findOneAndUpdateAsync(match, {
            $set: {
    ...
    .then(function (upsert_res) {
        feed_data.items.forEach(function (post) {
            ...
            ret.push(m.collection('posts').updateOneAsync(match, {
                $set: {
                    meta_id: meta_id,
                    guid: post.guid,
                    user_id: user_id,
